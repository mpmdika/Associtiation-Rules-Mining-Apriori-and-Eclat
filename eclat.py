...
def checkPycuda(text_area):
	try:
		...
		CUDA_FLAG = False
		viewProgress("Failed to import Pycuda!\nMachine does not support GPU acceleration.", text_area)
...
"""
def compute_vertical_bitvector_data(data, use_CUDA, text_area):
...
class eclat_runner:
	...
	def __init__(self, num_trans, min_support, use_CUDA, block, thread, text_area, use_optimal=True):
		...
		if self.use_CUDA and not self.use_optimal:
			...
			viewProgress("Using Block ="+str(self.block), self.text_area)
			viewProgress("Using Grid ="+str(self.grid), self.text_area)
		elif self.use_CUDA:
			viewProgress("Accelerating Eclat computation with GPU!", self.text_area)
		else:
			viewProgress("Not using GPU for acceleration.", self.text_area)
	...
	def run(self, prefix, supportK):
		...
		viewProgress('Running Eclat in recursive: number of itemsets found:'+str(len(self.support_list)), self.text_area)
		while supportK:
...
"""
	...
	- Output:
		...
		- support_list: a dictionary recording the supports for each itemset in L
"""
def eclat(data, min_support, iterative=True, use_CUDA=False, block=None, thread=None, text_area=None):
	tracemalloc.start()
	...
	if iterative and not use_CUDA:
		...
		vb_data, idx2item = compute_vertical_bitvector_data(data, use_CUDA=False, text_area=text_area)
		L1, support_list = compute_L1(vb_data, idx2item, num_trans, min_support)
		L = [L1]
		k = 1
		
		while True:
			viewProgress('Menjalankan Algoritma Apriori: Iterasi ke- '+str(k)+'dengan '+str(len(L[-1]))+' itemsets in L'+str(k)+'...', text_area)
			k += 1
			...
			if len(LK) == 0:
				...
				viewProgress('enjalankan Algoritma Apriori: Iterasi ke- '+str(k-1)+'-th. Terminating ...', text_area)
				break
			else:
				L.append(LK)
	...
	#---recursive method---#
		...
		vb_data, idx2item = compute_vertical_bitvector_data(data, use_CUDA=use_CUDA, text_area=text_area)
		...
		#---eclat class runner---#
		eclat = eclat_runner(num_trans, min_support, use_CUDA, block, thread, text_area, use_optimal=True)
