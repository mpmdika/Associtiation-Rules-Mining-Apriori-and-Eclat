...
def run_algorithm(data, mode, support, iterative):
	...
	iterative = True
	if mode == 'apriori':
		viewProgress('Menjalankan Algoritma Apriori dengan support '+str(support)+' dengan Records : '+str(np.shape(data)))
		result = apriori(data, support, text_area)
		return result
	elif mode == 'eclat':
		viewProgress('Menjalankan Algoritma Eclat dengan support '+str(support)+' dengan Records : '+str(np.shape(data)))
...
def write_result(result, result_path):
	if len(result[0]) == 0: viewProgress('Total Frequent Itemset 0, Silahkan Coba dengan nilai minimum support yang lebih kecil!',text_area)
